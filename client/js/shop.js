...
function renderMaterialsInventory() {
    ...
    if (!window.bosses) {
        ...
        if (bossesData) window.bosses = JSON.parse(bossesData);
    }
...
function upgradeOriginalWeaponUI(weapon) {
    ...
    const beforeAbilityCount = (weapon.uniqueAbilities || []).length;
    ...
    if (player.equippedWeapon && player.equippedWeapon.id === weapon.id) {
        player.equippedWeapon = upgraded;
    }
    player.coins -= cost;
    
    localStorage.setItem("player", JSON.stringify(player));
    let message = `${weapon.name} を強化しました！倍率: ${upgraded.multiplier.toFixed(3)}x`;
    if ((upgraded.uniqueAbilities || []).length > beforeAbilityCount) {
        const newAbility = upgraded.uniqueAbilities[upgraded.uniqueAbilities.length - 1];
        message += `\n\n★tier4固有能力「${newAbility.name}」を獲得した！★`;
    }
    alert(message);
    renderOriginalWeapons();
    renderInventory();
