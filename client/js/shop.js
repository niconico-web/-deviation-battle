...
function initShop() {
    ...
    // オーブ合成モーダル
    const synthesizeOrbBtn = document.getElementById('synthesizeOrbBtn');
    if (synthesizeOrbBtn) {
        synthesizeOrbBtn.onclick = () => {
            const player = getPlayerData();
            if (!player || !player.orbs || player.orbs.length < 2) {
                alert('合成するにはオーブが2つ以上必要です。');
                return;
            }
            
            const orb1Index = parseInt(prompt('1つ目のオーブの番号を入力してください (1から):')) - 1;
            const orb2Index = parseInt(prompt('2つ目のオーブの番号を入力してください (1から):')) - 1;
            
            if (isNaN(orb1Index) || isNaN(orb2Index) || orb1Index === orb2Index ||
                !player.orbs[orb1Index] || !player.orbs[orb2Index]) {
                alert('無効な選択です。');
                return;
            }
            
            const orb1 = player.orbs[orb1Index];
            const orb2 = player.orbs[orb2Index];
            
            if (!confirm(`以下のオーブを合成しますか？\n1: ${getOrbDisplayName(orb1)}\n2: ${getOrbDisplayName(orb2)}`)) {
                return;
            }
            
            const result = synthesizeOrbs(player, orb1.id, orb2.id);
            if (!result.ok) {
                alert(result.message);
                return;
            }
            ...
            localStorage.setItem("player", JSON.stringify(result.player));
            alert(`オーブを合成しました！\n新しいオーブ: ${getOrbDisplayName(result.newOrb)}`);
            renderOrbInventory();
            updateStatus(result.player);
        };
    }
