...
// があるため、控えめな2倍とする。
// 【注意】通常のボットバトルは「モンスターごとの固定の合計ステータス」方式（monster-stats.js）に
// 変更されたが、ダンジョンは「階層とプレイヤーの実ステータスに応じて敵が強くなる」設計のため、
// 従来どおりプレイヤーの実ステータス基準で計算している。
    ...
    const html = `
        <div class="dungeon-info-panel">
            ...
            <div class="dungeon-info-details">
                ...
                武器オーブスロット追加チケットのいずれかが手に入ります。宝箱から出るオーブは
                コモン～神話のレア度が付き、レア度が高いほどランダムな追加ステータス（付与効果）が
                多く・大きく乗ります。</p>
